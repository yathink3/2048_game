...
const Game = ({ CROSS_NUMBER = 4, WINNING_NUMBER = 2048 }: { CROSS_NUMBER?: number; WINNING_NUMBER?: number }) => {
  ...
  return (
    <div className='fixed md:relative flex flex-col min-h-screen min-w-full justify-evenly items-center border-0 focus:outline-none noselect dark:bg-gray-800'>
      {prompt && <GameOverPrompt title='Guide' description={`Welcome to the 2048 game! In this exciting game, your goal is to strategically move cards using arrow keys. When two cards with the same number collide, they merge into a double. Keep merging and planning your moves to reach the final card with the number 2048. Good luck and enjoy the game!`} actions={<Button name='Continue Game' handleClick={()= setPrompt(false); } />} />}
